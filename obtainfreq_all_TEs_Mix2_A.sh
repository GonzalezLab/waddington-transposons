...
#SBATCH --job-name=obtain_freq_Mix2_A
#SBATCH --cpus-per-task=4
#SBATCH --partition=normal
#SBATCH -o logs/obtain_freq_Mix2_A_%a.out
#SBATCH -e logs/obtain_freq_Mix2_A_%a.err  
...
# sbatch --array=1-1142%50 obtainfreq_all_TEs_Mix2_A.sh
...
sed -i "/$TE\t$chr\t$start\t$end/d" $DIR/ANALYSES/post_analysis/frequency_insertions_all_${class}_$type.tab
sed -i "/$TE\t$chr\t$start\t$end/d" $DIR/ANALYSES/post_analysis/fisher_PopTE2_all_${class}_$type.tab
sed -i "/$TE\t$chr\t$start\t$end/d" $DIR/ANALYSES/post_analysis/fisher_TEMP2_all_${class}_$type.tab
sed -i "/$TE\t$chr\t$start\t$end/d" $DIR/ANALYSES/post_analysis/FC_PopTE2_all_${class}_$type.tab
sed -i "/$TE\t$chr\t$start\t$end/d"  $DIR/ANALYSES/post_analysis/FC_TEMP2_all_${class}_$type.tab
